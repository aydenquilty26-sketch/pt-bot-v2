...
_client = TradingClient(config.API_KEY, config.API_SECRET, paper=config.IS_PAPER)
...
def submit_buy(ticker: str, position_size_usd: float, last_price: float) -> dict:
    ...
    if qty < 1:
        return {"success": False, "reason": "position size below 1 share", "order_id": None}
    ...
    try:
        ...
        return {"success": True, "reason": "submitted", "order_id": str(order.id)}
    except Exception as e:
        return {"success": False, "reason": f"broker error: {e}", "order_id": None}
